...
- `zt apply` now remembers the content of the manifest it last applied and detects if `zt.yaml` changed on disk since then — before, `zt apply` trusted whatever manifest file it was pointed at with no way to notice an unexpected edit. On a mismatch it prints a diff and refuses to proceed until re-run with the new `--force` flag
